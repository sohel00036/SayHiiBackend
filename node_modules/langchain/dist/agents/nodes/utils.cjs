...
async function initializeMiddlewareStates(middlewareList, state) {
	...
	for (const middleware of middlewareList) {
		...
		*/
		...
		const modifiedSchema = (0, _langchain_core_utils_types.interopZodObjectMakeFieldsOptional)(zodSchema, (key) => key.startsWith("_"));
		const parseResult = await (0, _langchain_core_utils_types.interopSafeParseAsync)(modifiedSchema, state);

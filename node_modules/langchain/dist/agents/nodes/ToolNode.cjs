...
var ToolNode = class extends require_RunnableCallable.RunnableCallable {
	...
	*/
	...
	async run(state, config) {
		...
		for (const output of outputs) if ((0, _langchain_langgraph.isCommand)(output)) {
			if (output.graph === _langchain_langgraph.Command.PARENT && Array.isArray(output.goto) && output.goto.every((send) => isSend(send))) {

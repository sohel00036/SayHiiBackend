			...
			if (structuredResponseFormat?.type === "native") {
				...
				*/
				if (!response.tool_calls || response.tool_calls.length === 0) {
					const schemaTitle = typeof structuredResponseFormat.strategy.schema?.title === "string" ? structuredResponseFormat.strategy.schema.title : "providerStrategy";
					throw new require_errors.StructuredOutputParsingError(schemaTitle, ["Model output did not satisfy the provided response schema."]);
				}
				return response;
			}
